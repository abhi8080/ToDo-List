import {storeProjectsAndToDos, fetchFromStorage, updateProjects} from "./localStorage"
...
document.body.addEventListener('click', (e) => {
    ...
    else if(e.target.classList.contains("done-button"))
    {
        ...
        let parentContainer = document.getElementById(`${id}container${project.id}`);
        const paragraphs = parentContainer.getElementsByClassName('details');
        Array.prototype.forEach.call(paragraphs, function(paragraph){
            if(!paragraph.classList.contains('detailDueDate')) {
...
function updateToDoList(id, parentContainer,project) {
    ...
    let priority = document.getElementById(`${id}priority${project}`);
    let dueDate = document.getElementById(`${id}dueDate${project}`);

    title.innerHTML = parentContainer.querySelector('.detailTitle').innerHTML;
    dueDate.innerHTML = parentContainer.querySelector('.detailDueDate').value.replace("T","&nbsp &nbsp &nbsp");
    priority.innerHTML = parentContainer.querySelector('.detailPriority').innerHTML;
}
...
function deleteProject(project) {
 ...
 function removeProjectFromDropDownMenu(project) {
    document.querySelectorAll(`option[value=${project}]`).forEach(element => element.remove());
 }
...
function createNewToDo(){
    ...
    const toDo = new ToDo(document.getElementById('title').value, document.getElementById('description').value, document.getElementById('dueDate').value, document.getElementById('priority').value);

   const id = document.getElementById(`${project}`).getElementsByClassName("container").length;
...
node_4.innerHTML = toDo.priority;
